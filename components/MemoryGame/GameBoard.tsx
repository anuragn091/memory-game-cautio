...
import { Tile, checkMatch, calculateBoardSize, ICONS } from '../../utils/memoryGame';
...
export default function GameBoard({ board, setBoard, onWin, darkMode }: Props) {
  ...
  return (
    <div className="flex justify-center items-center w-full">
      <div className={`grid gap-2 sm:gap-4 max-w-fit`} style={{ gridTemplateColumns: `repeat(${boardSize.cols}, minmax(0, 1fr))` }}>
        {board.map((tile, idx) => (
          <TileComponent key={tile.id} tile={tile} onClick={handleTileClick} index={idx} darkMode={darkMode} />
        ))}
      </div>
    </div>
  );
